...
[[nodiscard]] auto read_slot(std::span<const std::byte> page, u16 slot_idx) -> EdbResult<Slot> {
...
[[nodiscard]] auto validate_page_size(std::span<const std::byte> page) -> EdbStatus {
...
auto initialize_page(std::span<std::byte> page, u64 page_id) -> EdbStatus {
...
auto slot_count(std::span<const std::byte> page) -> EdbResult<u16> {
    if (auto status = validate_page_size(page); !status) {
...
auto free_space(std::span<const std::byte> page) -> EdbResult<usize> {
    if (auto status = validate_page_size(page); !status) {
        return std::unexpected(status.error());
    }
...
auto can_insert(std::span<const std::byte> page, usize tuple_size) -> EdbResult<b8> {
...
auto insert_tuple(std::span<std::byte> page, std::span<const std::byte> tuple) -> EdbResult<u16> {
    ...
    if (!can_fit->value) {
        return std::unexpected(EdbError::OutOfMemory);
    }
...
auto delete_tuple(std::span<std::byte> page, u16 slot_idx) -> EdbStatus {
...
auto is_live_slot(std::span<const std::byte> page, u16 slot_idx) -> EdbResult<b8> {
    auto slot = read_slot(page, slot_idx);
    if (!slot) {
        return std::unexpected(slot.error());
    }
