...
TEST(Log, LevelNameDebug) { EXPECT_EQ(log_level_name(LogLevel::Debug), "DEBUG"); }
TEST(Log, LevelNameInfo)  { EXPECT_EQ(log_level_name(LogLevel::Info),  "INFO "); }
TEST(Log, LevelNameWarn)  { EXPECT_EQ(log_level_name(LogLevel::Warn),  "WARN "); }
TEST(Log, LevelNameError) { EXPECT_EQ(log_level_name(LogLevel::Error), "ERROR"); }
TEST(Log, LevelNameNone)  { EXPECT_EQ(log_level_name(LogLevel::None),  "NONE "); }
...
TEST(Log, SmokeDoesNotCrash) {
    ...
    EXPECT_NO_FATAL_FAILURE(
        edb_log(LogLevel::Info, "test", "info message")
    );
    EXPECT_NO_FATAL_FAILURE(
        edb_log(LogLevel::Error, "test", "error {}", "detail")
    );
}
